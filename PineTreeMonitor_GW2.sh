...
#Deine server properties for rpi-ws2812-server instance handling GW 2
...
tcpdump -nnvvvi $FACE not port 22 and not port $PORT | while read b; do #Main loop runs every packet caught by TCPdump, ignores 22 and own application port (some verbosity required for protocol, added all the verbosity)
    #echo $b  #debug ouput
    if [[ "$b" == *" TCP "*" S, "*" win "* ]]; then #checks for TCP, should only match SIN and I guess SINACK
    ...
    elif [[ "$b" == *" UDP "* ]]; then #should only match UDP
      ...
      echo "render;" 1>&3
      c="1"
    elif [[ "$b" == *" arp "* ]]; then #should only match arp requests and replies
      if [[ "$c" == "2" ]]; then
        echo "rotate $CHANNEL,1,1,000000;" 1>&3
        echo " "
      fi
